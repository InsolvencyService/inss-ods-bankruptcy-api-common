...
{
    public class PDFForm2_68_DebtHistoryVm
    {
        public PDFForm2_68_DebtHistoryVm()
        {
        ...
        public PDFForm2_68_DebtHistoryVm(ApplicantIdentity applicantIdentity)
        {
            BankruptcyHistory = new List<BankruptcyHistoryVm>();
            IndividualVoluntaryAgreementHistory = new List<IndividualVoluntaryAgreementHistoryVm>();
            PreferentialPaymentHistory = new List<PreferentialPaymentHistoryVm>();
                        ...
                        if (preferentialPaymentsMade.Any())
                        {
                            foreach (var preferentialPayment in preferentialPaymentsMade)
                            {
                                PreferentialPaymentHistory.Add(new PreferentialPaymentHistoryVm()
                                {
                                    ...
                                    AssetName = preferentialPayment.PreferentialPaymentDetail.AssetName,
                                    CreditorAddress = preferentialPayment.PreferredCreditor.CreditorAddress.FullAddress,
                                    RelationShipToYou = !string.IsNullOrWhiteSpace(preferentialPayment.PreferredCreditorRelationship.RelationshipToYou)
                                                            ? preferentialPayment.PreferredCreditorRelationship.RelationshipToYou 
                                                            : preferentialPayment.PreferredCreditorRelationship.RelationshipToYouOther
                                });
                            }
                        }
            ...
            if (applicantIdentity.DebtIncurredDetail != null && applicantIdentity.DebtHistory != null)
            {
                ...
                {
                    reasons.Append(applicantIdentity.DebtIncurredDetail.BusinessLossOfCustomerOrMarket ? "<br /><br />" + DebtIncurredDetailResources.Debt_DebtIncurredDetail_BusinessLossOfCustomerOrMarket_Reason_Label : "");
                }
    ...
    public class IndividualVoluntaryAgreementHistoryVm
    {
        public DateTime? Date { get; set; }
    }

    public class BankruptcyHistoryVm
    {
    ...
    public class PreferentialPaymentHistoryVm
    {
        public DateTime? Date { get; set; }
