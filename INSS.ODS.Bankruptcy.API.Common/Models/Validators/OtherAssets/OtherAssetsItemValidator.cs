...
namespace INSS.ODS.Bankruptcy.API.Common.Models.Validators.OtherAssets
{
    public class OtherAssetsItemValidator : AbstractValidator<IOtherAssetsItem>
    {
        public OtherAssetsItemValidator()
        {
            RuleFor(x => x.Amount)
                .NotEmpty()
                .WithMessage(ItemsResources.OtherAssets_Items_Amount_Error_Required)
                .When(x => !String.IsNullOrWhiteSpace(x.Description));

            RuleFor(x => x.Amount)
                .Must(x => x > 0)
                .WithMessage(ItemsResources.OtherAssets_Items_Amount_Error_Minimum)
                .When(x => x.Amount != null);

            RuleFor(x => x.Description)
                .NotEmpty()
                .WithMessage(ItemsResources.OtherAssets_Items_Description_Error_Required)
                .When(x => x.Amount != null);

            RuleFor(x => x.Description)
                .Must(x => x.Length <= 100)
                .WithMessage(ItemsResources.OtherAssets_Items_Description_Error_MaxLength)
                .When(x => !String.IsNullOrWhiteSpace(x.Description));
