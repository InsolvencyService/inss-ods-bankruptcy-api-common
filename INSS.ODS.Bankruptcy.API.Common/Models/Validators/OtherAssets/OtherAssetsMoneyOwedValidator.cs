...
namespace INSS.ODS.Bankruptcy.API.Common.Models.Validators.OtherAssets
{
    public class OtherAssetsMoneyOwedValidator : AbstractValidator<IOtherAssetsMoneyOwed>
    {
        public OtherAssetsMoneyOwedValidator()
        {
            RuleFor(x => x.Amount)
                .NotEmpty()
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_Amount_Error_Required)
                .When(x => !String.IsNullOrWhiteSpace(x.Reason) || !string.IsNullOrWhiteSpace(x.WhoOwesYou));

            RuleFor(x => x.Amount)
                .Must(x => x > 0)
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_Amount_Error_Minimum)
                .When(x => x.Amount != null);

            RuleFor(x => x.Reason)
                .NotEmpty()
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_Reason_Error_Required)
                .When(x => x.Amount != null || !String.IsNullOrWhiteSpace(x.WhoOwesYou));

            RuleFor(x => x.WhoOwesYou)
                .NotEmpty()
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_WhoOwesYou_Error_Required)
                .When(x => x.Amount != null || !String.IsNullOrWhiteSpace(x.Reason));

            RuleFor(x => x.Reason)
                .Must(x => x.Length <= 100)
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_Reason_Error_MaxLength)
                .When(x => !String.IsNullOrWhiteSpace(x.Reason));

            RuleFor(x => x.WhoOwesYou)
                .Must(x => x.Length <= 100)
                .WithMessage(MoneyOwedResources.OtherAssets_MoneyOwed_WhoOwesYou_Error_MaxLength)
