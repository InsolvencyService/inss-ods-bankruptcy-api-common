...
namespace INSS.ODS.Bankruptcy.API.Common.Models.Validators.OtherAssets
{
    public class OtherAssetsPossessionValidator : AbstractValidator<IOtherAssetsPossession>
    {
        public OtherAssetsPossessionValidator()
        {
            RuleFor(x => x.Amount)
                .NotEmpty()
                .WithMessage(PossessionsResources.OtherAssets_Possessions_Amount_Error_Required)
                .When(x => !String.IsNullOrWhiteSpace(x.Description));

            RuleFor(x => x.Amount)
                .Must(x => x >= 500)
                .WithMessage(PossessionsResources.OtherAssets_Possessions_Amount_Error_Minimum)
                .When(x => x.Amount != null);

            RuleFor(x => x.Description)
                .NotEmpty()
                .WithMessage(PossessionsResources.OtherAssets_Possessions_Description_Error_Required)
                .When(x => x.Amount != null);

            RuleFor(x => x.Description)
                .Must(x => x.Length <= 100)
                .WithMessage(PossessionsResources.OtherAssets_Possessions_Description_Error_MaxLength)
                .When(x => !String.IsNullOrWhiteSpace(x.Description));
