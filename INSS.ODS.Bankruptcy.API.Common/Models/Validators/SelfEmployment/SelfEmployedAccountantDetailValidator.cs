    ...
    public class SelfEmployedAccountantDetailValidator<TAddress> : AbstractValidator<IAccountantDetail<TAddress>> where TAddress : IAddress
    {
        public SelfEmployedAccountantDetailValidator()
        {
            RuleFor(x => x.AccountantName)
                ...
                .NotEmpty()
                .WithMessage(AccountantDetailResources.AccountantDetail_AccountantName_Required)
                .Length(3, 30)
                .WithMessage(AccountantDetailResources.AccountantDetail_AccountantName_Length);

            RuleFor(x => x.DateStarted)
                .Cascade(CascadeMode.StopOnFirstFailure)
                .NotEmpty()
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStarted_Required)
                .GreaterThanOrEqualTo(new DateTime(1900, 1, 1))
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStopped_After_1900)
                .LessThan(DateTime.Today)
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStarted_NotInFuture);

            RuleFor(x => x.DateStopped)
                .GreaterThanOrEqualTo(new DateTime(1900, 1, 1))
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStopped_After_1900)
                .LessThan(DateTime.Today)
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStopped_NotInFuture)
                .GreaterThanOrEqualTo(x => x.DateStarted)
                .WithMessage(AccountantDetailResources.SelfEmployment_AccountantDetail_DateStopped_Not_Before_StartDate)
